...
function addImage() {
    for (var i = 0; i < 9; i++) {
        saiyanPic = document.getElementsByClassName("classimg");
        saiyanPic.src = "img/img" + (i + 1) + ".jpg";
        saiyanPic.id = (i + 1);
        saiyanPic.addEventListener("click", function() {
            addHeart(this.id);
        });
        favorite = document.getElementsByClassName("extra");
        favorite.id = "extra_" + (i + 1);
    }
}

function addHeart(id) {
    clearImages();
    favorite = document.getElementById("extra_" + id);
    favorite.src = "img/heart.png";
}
